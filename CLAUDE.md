...
- [x] ログイン・新規登録ページのPC幅調整（max-width制約を削除しフルワイド化）
- [x] 管理画面レスポンシブ改善（768px問題修正、モバイルナビアイコン化、タップ領域拡大）
- [x] メガメニュー改善（カテゴリ画像プレースホルダー追加、オーバーレイ表示に変更）

### フロントエンドで対応可能
- [x] サイズ・ケアガイドページの作成（サイズテーブル、素材別ケアガイド、ケアシンボル）
- [x] Aboutページの改善（ヒーロー、ブランドストーリー、素材紹介、CTA追加）
- [ ] 商品比較機能
- [ ] OGP画像・ファビコンの設定
